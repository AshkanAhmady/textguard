...
6. **Developer integration slice — 🟡 in progress.** The initial CLI milestone is feature-complete for current scope. The VS Code extension is publicly available on Marketplace, and the browser Playground now provides preset-based scanning plus structured Explain metadata without adding dependencies to the library workspace.
...
| M6 — Future Integrations | 🟡 Partial | `@textguard/cli` is complete for its initial scope. VS Code is publicly released, and the browser Playground supports interactive scan/preset usage plus structured Explain visualization with isolated CI. Chrome and AI integrations remain later. |
...
**Playground Explain validation → debug/timeline visualization → deployment → adoption feedback.**
