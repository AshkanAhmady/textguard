...
| M6 — Future Integrations | 🟡 Partial | `@textguard/cli` is complete for its initial scope. VS Code is publicly released, and the browser Playground is feature-complete for the current milestone with interactive scanning, built-in examples, shareable URLs, structured Explain visualization, Debug event ordering, timeline projection, Enterprise detector controls, and GitHub Pages deployment automation. Chrome and AI integrations remain later and should only start when adoption evidence justifies them. |
