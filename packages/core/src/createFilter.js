...
export function createFilter(options) {
    ...
    const { dictionaries = [], customWords = [], whitelist = [], mask = "*", leetspeakMapping = {}, faLookalikesMapping = {} } = options;
        ...
        for (const entry of sortedEntries) {
            ...
            if (entry.word instanceof RegExp) {
                ...
                while ((match = regex.exec(text)) !== null) {
                    ...
                    if (!matchedText)
                        break; // جلوگیری از حلقه‌های بی‌نهایت در ریجکس‌های خاص با طول صفر
                    const start = match.index;
                    const end = start + matchedText.length;
                    // بررسی لیست سفید (وایت‌لیست) برای ریجکس
                    const isWhitelisted = whitelist.some((w) => w.toLowerCase() === matchedText.toLowerCase() ||
                        w.toLowerCase() === text.substring(start, end).toLowerCase());
                    if (isWhitelisted)
                        continue;
                    // جلوگیری از تداخل با کلماتی که قبلاً بزرگ‌تر بوده‌اند و دیتکت شده‌اند
                    const isOverlapped = matches.some((m) => (start >= m.start && start < m.end) || (end > m.start && end <= m.end));
                    if (!isOverlapped) {
                        matches.push({
